/* ==========================================================================
   DROXO SHOP — app.js  v4  (All 7 features)
   1. Learn More Modal + 3-platform ordering
   2. Category filter tabs + combined search
   3. Admin: category select + description field
   4. Discord Webhook (automated order embed)
   5. Skeleton loader animation
   6. SEO meta tags (in HTML head)
   7. Full RTL Arabic UI
   ========================================================================== */
...
// Replace with your real webhook, e.g.:
...
/* ══════════════════════════════════════════════════════════════════════════
   ② STORAGE HELPERS
   ══════════════════════════════════════════════════════════════════════════ */
const getProducts   = () => JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.PRODUCTS) || "[]");
const saveProducts  = (p) => localStorage.setItem(CONFIG.STORAGE_KEYS.PRODUCTS, JSON.stringify(p));
const getPromoCodes = () => JSON.parse(localStorage.getItem(CONFIG.STORAGE_KEYS.PROMOS)   || "[]");
const savePromoCodes= (p) => localStorage.setItem(CONFIG.STORAGE_KEYS.PROMOS,   JSON.stringify(p));
...
/* ══════════════════════════════════════════════════════════════════════════
   ⑤ CATEGORY FILTER STATE (Feature 2)
   ══════════════════════════════════════════════════════════════════════════ */
...
/** Builds one skeleton card element */
...
/** Shows skeletons, then after delay renders real cards */
function renderWithSkeleton(renderFn) {
  /* Show skeletons */
  ...
  /* After delay, swap in real content */
  setTimeout(() => {
...
/** Core render — always called inside renderWithSkeleton callback */
function renderProductsGrid(filterText = "") {
  renderWithSkeleton(() => _doRender(filterText));
}

function _doRender(filterText = "") {
...
/* ══════════════════════════════════════════════════════════════════════════
   FEATURE 4: DISCORD WEBHOOK — send order embed to admin channel
   ══════════════════════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════════════════════
   BUILD PRODUCT CARD (Feature 1 + inline SVG icons)
   ══════════════════════════════════════════════════════════════════════════ */
  ...
  card.innerHTML = `
    ...
    <div class="product-body">
      ...
      <div class="order-buttons">
        ...
        <!-- Discord: intentionally never disabled -->
...
/* ══════════════════════════════════════════════════════════════════════════
   ORDER MESSAGE + OPENER (includes Discord Webhook trigger)
   ══════════════════════════════════════════════════════════════════════════ */
function buildOrderMessage(product, finalPrice, appliedPromo) {
  ...
  if (appliedPromo) msg += `🎟️ كود الخصم: ${appliedPromo.code} (-${appliedPromo.discountValue}%)\n`;
  msg     += `----------------------------\n`;
...
function openOrderLink(platform, product, finalPrice, appliedPromo) {
  if (platform === "discord") {
    /* Feature 4: fire webhook then redirect */
...
/* ══════════════════════════════════════════════════════════════════════════
   PRODUCT DETAILS MODAL (Feature 1)
   ══════════════════════════════════════════════════════════════════════════ */
...
function openProductDetailsModal(product, finalPrice) {
  ...
  modalDiscordBtn.disabled = false; /* always active */
...
/* ══════════════════════════════════════════════════════════════════════════
   ADMIN — PRODUCT FORM (Feature 3: category + description)
   ══════════════════════════════════════════════════════════════════════════ */
...
function renderAdminProductTable() {
  ...
  if (products.length === 0) {
    productsTableBody.innerHTML =
      `<tr><td colspan="11" style="text-align:center;color:#6b6b6b;padding:24px;">
